	...
	* Custom walker class.
	...
	class Boombox_Walker_Badges_Nav_Menu extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$classes .= 'post_tag' == $item->object ? apply_filters( 'boombox_badge_wrapper_advanced_classes', ' category', 'post_tag', $item->object_id ) : '';
			...
			} elseif ( in_array( $item->object, array( 'category', 'post_tag') ) ) {
				$category_icon_name = boombox_get_term_icon_name( $item->object_id, $item->object );
				$icon               = ! empty( $category_icon_name ) ? '<i class="icon icon-' . $category_icon_name . '"></i>' : '';
			} elseif ( boombox_is_trending( $item->object_id ) ) {
				$trending_icon_name = boombox_get_trending_icon_name( $item->object_id );
				$icon               = ! empty( $trending_icon_name ) ? '<i class="icon icon-' . $trending_icon_name . '"></i>' : '';
			}
			...
			$item_output = sprintf( '%1$s<a%2$s><span class="circle">%3$s</span><span class="text">%4$s%5$s%6$s</span></a>%7$s',
				$args_before,
				$attributes,
				$icon,
				$args_link_before,
