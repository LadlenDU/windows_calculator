...
if ( ! class_exists( 'Boombox_Widget_Picked_Posts' ) ) {
	...
	class Boombox_Widget_Picked_Posts extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			 ...
			 */
			$query = boombox_get_posts_query( $condition, $time_range, $categories, $tags, $number );
									...
									<div class="post-thumbnail">
										...
										<a href="<?php echo esc_url( get_permalink() ); ?>"
										   title="<?php echo esc_attr( the_title_attribute() ); ?>">
											<?php the_post_thumbnail( 'boombox_image360x180' ); ?>
										</a>
										<?php endif; ?>
									</div>
								<div class="content">
		...
		public function update( $new_instance, $old_instance ) {
			...
			if ( isset( $new_instance['categories'] ) && is_array( $new_instance['categories'] ) ) {
				$categories = array_filter( $new_instance['categories'] );
				if ( ! empty( $categories ) ) {
					$instance['categories'] = esc_sql( $categories );
				}
			}

			$instance['tags'] = '';
			if ( isset( $new_instance['tags'] ) && is_array( $new_instance['tags'] ) ) {
				$tags = array_filter( $new_instance['tags'] );
				if ( ! empty( $tags ) ) {
					$instance['tags'] = esc_sql( $tags );
				}
			}
		...
		public function form( $instance ) {
			...
			$categories  = isset( $instance['categories'] ) ? esc_sql( $instance['categories'] ) : '';
			$tags        = isset( $instance['tags'] ) ? esc_sql( $instance['tags'] ) : '';
			$number      = isset( $instance['number'] ) ? absint( $instance['number'] ) : 5;
			$numbered    = isset( $instance['numbered'] ) ? ( bool ) $instance['numbered'] : false;
			$show_date   = isset( $instance['show_date'] ) ? (bool) $instance['show_date'] : false;
			$show_author = isset( $instance['show_author'] ) ? (bool) $instance['show_author'] : false;

			$conditions_choices = boombox_get_conditions_choices();
			$time_range_choices = boombox_get_time_range_choices();
			$category_choices   = boombox_get_category_choices();
			$tag_choices        = boombox_get_tag_choices(); ?>
			<p>
				<label
			...
			<p>
				<label
					for="<?php echo $this->get_field_id( 'tags' ); ?>"><?php esc_html_e( 'Tags:', 'boombox' ); ?></label>
				<?php if ( is_array( $tag_choices ) ): ?>
					<select id="<?php echo $this->get_field_id( 'tags' ); ?>"
					        name="<?php echo $this->get_field_name( 'tags' ); ?>[]" class="widefat" multiple="multiple">
						<?php foreach ( $tag_choices as $tag_slug => $tag_name ) :
							if ( is_array( $tags ) ) {
								$selected = selected( in_array( $tag_slug, $tags, true ), true, false );
							} else {
								$selected = selected( $tag_slug, $tags, false );
							} ?>
							<option
								value="<?php echo esc_attr( $tag_slug ); ?>" <?php echo $selected; ?>><?php echo esc_html( $tag_name ); ?></option>
						<?php endforeach; ?>
					</select>
				<?php endif; ?>
			</p>
