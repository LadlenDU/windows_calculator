...
if ( ! class_exists( 'Boombox_Widget_Trending_Posts' ) ) {
	...
	class Boombox_Widget_Trending_Posts extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			$icon 		 = ( $type == 'trending' ) ? boombox_get_theme_option( 'design_badges_trending_icon' ) : $type;
			 ...
			 */
			$query = boombox_get_trending_posts( $type, $number, 1, false, 'none', 1, 'none', 1, true );

			if ( null != $query && $query->have_posts() ) :
				?>
				<?php echo $args['before_widget']; ?>
				<?php if ( $title ) {
					$type_icon = $type ? '<i class="icon icon-' . $icon . '"></i>' : '';
