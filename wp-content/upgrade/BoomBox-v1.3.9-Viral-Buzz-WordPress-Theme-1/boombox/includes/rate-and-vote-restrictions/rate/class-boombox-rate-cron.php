	...
	class Boombox_Rate_Cron
	{
		...
		static protected function update_ratings(Boombox_Rate_Job $job)
		{
			...
			if ($criteria->has_count_column()) {
				$aggregation = $wpdb->prepare(' SUM(`%1$s`) ', $criteria->get_count_column_name());
			} else {
				$aggregation = ' COUNT(*) ';
			}

			$query = $wpdb->prepare('
				SELECT `rate_table`.`%1$s`, "%2$s" AS `rate_name`, ' . $aggregation . ' AS `rate_count`
				FROM `%3$s` AS `rate_table`
				JOIN `' . $wpdb->posts . '` AS `rate_posts`
				    ON `rate_posts`.`ID` = `rate_table`.`%4$s`
				    AND `rate_posts`.`post_type` In( ' . $post_types . ' )
				    AND `rate_posts`.`post_status` In( ' . $post_statuses . ' )
				WHERE ' . $where . '
				GROUP BY `rate_table`.`%1$s`
				HAVING `rate_count` >= %5$d
				ORDER BY `rate_count` DESC
				LIMIT %6$d
        ', $criteria->get_post_id_column_name(),
				$meta_key,
				$criteria->get_table_name(),
				$criteria->get_post_id_column_name(),
				$job->get_min_count(),
				$job->get_limit()
			);
		...
		public static function filter_rate_where($where, Boombox_Rate_Job $job)
		{
			...
			if (0 < $time_range->get_day_count()) {
				$where_fragment = $wpdb->prepare(' AND `%1$s` > DATE_ADD( CURDATE( ), INTERVAL -%2$d DAY ) ',
					$criteria->get_date_column_name(),
		...
		{
			return !!get_post_meta($post_id, static::get_meta_key($job), true);
		}
	}
}
