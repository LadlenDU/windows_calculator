...
jq(document).ready( function() {
	...
	// cover image show/hide functionality on change
	if( jq("#header-cover-image").length ) {
		window.bp.CoverImage.Attachment.on('change:url', function (data) {
			var cover_image_holder = jq("#header-cover-image").parents(".bbp-wrapper");
			if (data.attributes.url) {
				cover_image_holder.removeClass("no-item-image");
			} else {
				cover_image_holder.addClass("no-item-image");
			}
		});
	}
